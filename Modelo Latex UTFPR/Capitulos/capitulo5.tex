...
Este capítulo apresentará os resultados mais relevantes das comparações. É categorizado por tipo de \emph{collider} e em seguida por tipo de cenário (\emph{Pre-Runtime} e \emph{Runtime}).
...
De acordo com a pesquisa de campo realizada, cerca de 70\% do uso das colisões são de \emph{Box} e \emph{Circle Collider} para suas cenas em 2D como mostra a Figura \ref{questionario_uso_colliders_2d}.
...
\begin{figure}[!htb]
	...
	\label{questionario_uso_colliders_2d}
\end{figure}

Em relação as mensagens usadas, há uma distribuição equilibrada quando o uso é de início e final de contato com a colisão ou trigger, como mostra a Figura \ref{questionario_uso_mensagens_2d}. Já enquanto o contato ainda está ocorrendo é pouco utilizado. O número médio de objetos numa cena podem ser encontrados na Figura \ref{questionario_media_2d}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.75\textwidth]{./Figuras/Questionario/resultado/05_mensagens_2D_mais.png}
	\caption[Frequência de uso de Colliders 2D]{Frequência de uso de \emph{Colliders} 2D.}
	\label{questionario_uso_mensagens_2d}
\end{figure}
