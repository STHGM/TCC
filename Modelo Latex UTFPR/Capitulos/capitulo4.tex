...
Cada cenário foi feito usando um dos quatro \emph{colliders} que foram mencionados antes, cada um em \emph{Pre-Runtime} e \emph{Runtime}.
...
O cenário \emph{Pre-Runtime} consiste de três elementos. O objeto com um tipo de \emph{collider} e \emph{rigid body} atribuídos a ele. Esse objeto é representado pelo personagem em azul.

Um objeto que permanece parado, com \emph{box collider} em todas as comparações, porém, não é atribuído \emph{rigid body} a ele, e sim \emph{trigger}, ou seja, objetos podem atravessá-lo, mas a detecção de colisão ainda ocorre. Este é representado pela plataforma que se encontra logo abaixo do personagem. 

A última plataforma em baixo, é um objeto estático, porém com um \emph{box collider}, ou seja, os objetos não podem atravessá-lo.
